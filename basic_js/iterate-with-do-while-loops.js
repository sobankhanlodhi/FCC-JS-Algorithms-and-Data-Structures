...
It is called a do...while loop because it will first do one pass of the code inside the loop 
no matter what, and then continue to run the loop while the specified condition evaluates to true.
...
look like [0, 1, 2, 3, 4]. However, what makes the do...while different from other loops is 
...
so we do not execute the code inside the loop. The result is that ourArray will end up 
with no values added to it, and it will still look like [] when all of the code in the 
example above has completed running. 
...
so we go to the code inside the curly braces and execute it. We will add a single element 
